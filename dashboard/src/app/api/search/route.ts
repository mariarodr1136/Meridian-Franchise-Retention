...
async function ensureIndex() {
  ...
  await buildSearchIndex({ studios, reviews, instructors });
}
...
export async function GET(req: NextRequest) {
  ...
  await ensureIndex();
  const results = await ftsSearch(q);
...
export async function POST() {
  ...
  const count = await buildSearchIndex({ studios, reviews, instructors });
